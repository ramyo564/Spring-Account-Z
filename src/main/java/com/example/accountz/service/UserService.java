...
import static com.example.accountz.type.ErrorCode.ALREADY_REGISTERED_EMAIL;
import static com.example.accountz.type.ErrorCode.INTERNAL_SERVER_ERROR;
import static com.example.accountz.type.ErrorCode.USER_NOT_FOUND;
import static com.example.accountz.type.ErrorCode.WRONG_PASSWORD;
...
public class UserService implements UserDetailsService {
  ...
  public UserEntity registerUser(UserDto.SignUp member)
    ...
    boolean exists = this.userRepository.existsByEmail(member.getEmail());
    if (exists) {
  ...
  public UserEntity authenticateUser(UserDto.SignIn member)
    ...
    var user = this.userRepository
        .findByEmail(member.getEmail()).orElseThrow(() ->
            new GlobalException(USER_NOT_FOUND));

    if (!this.passwordEncoder.matches(
        member.getPassword(), user.getPassword())) {
